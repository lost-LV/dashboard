...
(function () {
    ...
    const config = {
        ...
        minMarkerSize: 2, // Smaller minimum size
        maxMarkerSize: 10, // Larger maximum size
        minDollarValue: 100000, // Minimum dollar value to start scaling (100k)
        maxDollarValue: 1000000, // Maximum dollar value for scaling (1M)
        minDollarThreshold: 100000, // Only show liquidations when total > $100k
        minIndividualLiquidationSize: 1.0, // Minimum BTC size for individual liquidations (filter out smaller ones)
        minIndividualDollarValue: 100000, // Minimum dollar value for individual liquidations (approximately $100k)
        ...
        updateMarkers() {
            ...
            aggregated.forEach((agg, barTime) => {
                ...
                if (totalSellDollars >= config.minDollarThreshold) {
                    // Linear scaling between min and max size based on dollar value
                    // For values below minDollarValue, use minMarkerSize
                    // For values above maxDollarValue, use maxMarkerSize
                    ...
                    const sellSizeFactor = ((normalizedValue - config.minDollarValue) / valueRange) * sizeRange;
                ...
                if (totalBuyDollars >= config.minDollarThreshold) {
                    ...
                    const valueRange = config.maxDollarValue - config.minDollarValue;
                    const sizeRange = config.maxMarkerSize - config.minMarkerSize;
                    const buySizeFactor = ((normalizedValue - config.minDollarValue) / valueRange) * sizeRange;
    ...
    window.drawLiquidationMarkers = (ctx, bars, getYForPrice, barWidth, viewOffset) => {
        ...
        liquidationManager.markers.forEach(marker => {
            ...
            // Calculate arrow dimensions based on size - smaller base size but scales with value
            const size = marker.size * 4; // Smaller base multiplier
            const arrowWidth = size * 0.8; // Narrower arrows
            const arrowHeight = size * 1.5; // Shorter arrows
            const arrowHeadSize = size * 0.6; // Smaller arrow heads
            ...
            if (marker.text) {
                ...
                const fontSize = Math.max(10, Math.min(16, 10 + Math.log10(marker.size) * 2));
